...
function CasterState.load(unit_id)
    ...
    return {
        ...
        -- Explicit slot count for free assign/pick. nil = derive it from the
        -- caster's groups (the default). Set by casters built from nothing —
        -- e.g. the magic modification, where the slot count is a game setting.
        free_slots      = tonumber(wml.variables[prefix .. ".free_slots"]),
        -- Casts cost nothing (XP/HP/gold/attack are neither required nor spent).
        -- Honoured by the cast dialog, the AI, and the command-cast tags.
        free_casting    = wml.variables[prefix .. ".free_casting"] == true,
        -- Let this caster advance like a normal unit instead of being frozen at
        -- max XP - 1 (see the "pre advance" handler in utils.cfg).
        levels_normally = wml.variables[prefix .. ".levels_normally"] == true,
        -- Equipping a multi-skill (Summon, Bend, Polymorph, Astral Arms) unlocks
        -- all of its subskills at once, instead of earning them one by one as the
        -- campaign does. Set by the magic modification, where there is no
        -- campaign progression to earn them from.
        unlock_subskills = wml.variables[prefix .. ".unlock_subskills"] == true,
    }
end
...
function CasterState.save(data)
    ...
    wml.variables[prefix .. ".free_slots"]      = (tonumber(data.free_slots) or 0) > 0 and data.free_slots or nil
    wml.variables[prefix .. ".free_casting"]    = data.free_casting and true or nil
    wml.variables[prefix .. ".levels_normally"] = data.levels_normally and true or nil
    wml.variables[prefix .. ".unlock_subskills"] = data.unlock_subskills and true or nil
end
...
function CasterState.from_config(unit, cfg)
    ...
    return {
        ...
        free_slots      = tonumber(cfg.free_slots),
        free_casting    = (cfg.free_casting == true),
        levels_normally = (cfg.levels_normally == true),
        unlock_subskills = (cfg.unlock_subskills == true),
    }
end
