...
-- Unlocks every subskill of the caster's equipped multi-skills (Summon, Bend,
-- Polymorph, Astral Arms). Campaign casters earn those one at a time, so the
-- picker rightly shows the rest as "Locked"; a caster flagged unlock_subskills
-- (the magic modification, which has no campaign progression to earn them from)
-- gets the whole spell the moment it is equipped.
local function unlock_equipped_subskills(data)
    for _, spell_id in ipairs(data.equipped) do
        local spell = spell_index[spell_id]
        if spell and spell.subskills then
            for _, sub in pairs(spell.subskills) do
                if type(sub) == "table" and sub.id then CasterOps.unlock(data, sub.id) end
            end
        end
    end
end
...
wml_actions["magic_apply_selection"] = function(cfg)
    ...
    if data.unlock_subskills then unlock_equipped_subskills(data) end
...
---------------------------------------------------------------------------
-- CASTER REFUND
-- Gives back what a cast cost when it turned out to do nothing (no valid
-- target, no free hex) -- the counterpart of [caster_restore_casts], which gives
-- back the cast itself.
--
-- xp/hp/gold are catalogue costs, so a free-casting caster never paid them and
-- gets nothing back: refunding them unconditionally would MINT experience every
-- time a spell was cancelled. moves is not a catalogue cost (the spell's own
-- event spends it), so it is always returned.
--
-- Use: [caster_refund] xp=10 moves=1 [filter]...[/filter] [/caster_refund]
---------------------------------------------------------------------------

wml_actions["caster_refund"] = function(cfg)
    local units = wesnoth.units.find_on_map(require_filter(cfg, "caster_refund"))
    for _, u in ipairs(units) do
        local data = CasterState.load(u.id)
        if not (data and data.free_casting) then
            if cfg.xp   then u.experience = u.experience + tonumber(cfg.xp) end
            if cfg.hp   then
                u.hitpoints = math.min(u.max_hitpoints, u.hitpoints + tonumber(cfg.hp))
            end
            if cfg.gold then
                wesnoth.sides[u.side].gold = wesnoth.sides[u.side].gold + tonumber(cfg.gold)
            end
        end
        if cfg.moves then
            u.moves = math.min(u.max_moves, u.moves + tonumber(cfg.moves))
        end
    end
end
...
---------------------------------------------------------------------------
-- CASTER FREE SLOTS
-- Fixes how many slots the free-assign / free-pick picker offers, for casters
-- that have no groups to derive the count from (the magic modification builds
-- casters this way — its slot count is a game setting). 0 = derive from groups.
-- Use: [caster_free_slots] slots=4 [filter]...[/filter] [/caster_free_slots]
---------------------------------------------------------------------------

wml_actions["caster_free_slots"] = function(cfg)
    local units = wesnoth.units.find_on_map(require_filter(cfg, "caster_free_slots"))
    for _, u in ipairs(units) do
        local data = CasterState.load(u.id)
        if data then
            CasterOps.set_free_slots(data, cfg.slots)
            CasterState.save(data)
        end
    end
end
...
---------------------------------------------------------------------------
-- CASTER FREE CASTING
-- Spells cost this caster nothing: no XP/HP/gold/attack is required or spent,
-- by the cast dialog, the adaptive AI, or the command-cast tags below.
-- Use: [caster_free_casting] free_casting_allowed=yes [filter]...[/filter] [/caster_free_casting]
---------------------------------------------------------------------------

wml_actions["caster_free_casting"] = function(cfg)
    local units = wesnoth.units.find_on_map(require_filter(cfg, "caster_free_casting"))
    for _, u in ipairs(units) do
        local data = CasterState.load(u.id)
        if data then
            CasterOps.set_free_casting(data, cfg.free_casting_allowed == true)
            CasterState.save(data)
        end
    end
end
...
---------------------------------------------------------------------------
-- CASTER LEVELING
-- Lets a caster advance like a normal unit instead of being frozen one XP below
-- its advancement threshold (see the "pre advance" handler in utils.cfg).
-- Use: [caster_leveling] leveling_allowed=yes [filter]...[/filter] [/caster_leveling]
---------------------------------------------------------------------------

wml_actions["caster_leveling"] = function(cfg)
    local units = wesnoth.units.find_on_map(require_filter(cfg, "caster_leveling"))
    for _, u in ipairs(units) do
        local data = CasterState.load(u.id)
        if data then
            CasterOps.set_leveling(data, cfg.leveling_allowed == true)
            CasterState.save(data)
        end
    end
end
...
-- (a caster with free_casting set never pays either).
...
-- True when this cast must not be charged: either the caller said so (free=yes)
-- or the caster itself casts for free.
local function cast_is_free(cfg, data)
    return cfg.free == true or (data ~= nil and data.free_casting == true)
end
...
wml_actions["cast_spell"] = function(cfg)
    ...
    for _, u in ipairs(units) do
        ...
        if cast_gate_ok(cfg, data, spell_id) then
            if not cast_is_free(cfg, data) then apply_spell_cost(u.id, spell_id) end
