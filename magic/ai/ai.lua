...
local CasterOps   = wesnoth.require "~add-ons/Chasing_the_Light/magic/core/ops.lua"
...
M.profiles = wesnoth.require "~add-ons/Chasing_the_Light/magic/ai/ai_profiles.lua"
...
-- `data` is the caster's state table; a free-casting caster can afford anything.
local function can_afford(u, id, data)
    if data and data.free_casting then return true end
    local c = cost[id]
...
local function do_cast(cfg, pick)
    ...
    -- pick.free comes from the caster's own free_casting flag (set by choose_for /
    -- select_explicit), so a free-casting caster is never charged here either.
    local free = (cfg.free == true) or (pick.free == true)
...
local function choose_for(u, data)
    ...
    for _, spell_id in ipairs(data.equipped) do
        for _, cast_id in ipairs(castable_ids(spell_id)) do
            ...
            if prof and can_afford(u, cast_id, data) then
                ...
                if res then
                    ...
                    if not best or res.util > best.util then
                        best = { caster = u, spell = spell_id, cast = cast_id,
                                 ...
                                 move_to = hex, free = data.free_casting }
                    end
                end
            end
        end
...
local function select_explicit(cfg)
    for _, c in ipairs(casters_for(cfg)) do
        local u = c.unit
        if can_afford(u, cfg.spell, c.data) then
            if cfg.self then
                return { caster = u, spell = cfg.spell, self = true, free = c.data.free_casting }
            else
                local t = pick_target(u, cfg)
                if t then
                    return { caster = u, spell = cfg.spell, target = t, free = c.data.free_casting }
                end
            end
        end
    end
